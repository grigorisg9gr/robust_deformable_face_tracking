...
training_pos_im_1 = read_public_images(path_to_ibug, max_images=130, training_images=[], crop_reading=crop_reading, pix_thres=pix_thres, feat=feat) #130
training_pos_im_1 = read_public_images(path_to_helen, max_images=220, training_images=training_pos_im_1, crop_reading=crop_reading, pix_thres=pix_thres, feat=feat) #70
...
training_neg_im, cnt = pascal_db_non_faces(path_pascal_base, path_faces, max_loaded_images=500) #, 300
...
def process_clip(clip_name):
    ...
    if not check_path_and_landmarks(frames_path, clip_name, path_read_sh + clip_name): # check that paths, landmarks exist
        return
    training_pos_im_2 = load_images(list_frames, frames_path, path_init_sh, clip_name, max_images=150, feat=feat, crop_reading=crop_reading, pix_thres=130)
    list_frames = sorted(os.listdir(frames_path)) # re-read frames, in case they are cut, re-arranged in loading
    
    # concatenate positive examples (faces) 
    training_pos_im = training_pos_im_2 + training_pos_im_1 # put the person specific in the beginning
    del training_pos_im_2

    # Procrustes analysis
    print('\nPerforming Procrustes analysis for alignment of the training images in clip %s' % clip_name)
    from menpo.transform import GeneralizedProcrustesAnalysis as GPA
    gpa = GPA([i.landmarks['PTS'].lms for i in training_pos_im] )
    mean_shape = gpa.mean_aligned_shape()
    
    
    print('\nWarping the images and extracting patches in clip %s' % clip_name)
    ...
    # train SVM
    cl_1 = svm.LinearSVC(class_weight='auto', C=1).fit(tr_data, tr_class)
